...
import { Calendar, Clock, MapPin, Users, User, ChevronLeft, ChevronRight, Filter, LayoutGrid, List } from 'lucide-react';
                    ...
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <button
                                ...
                                className="flex items-center gap-1 px-3 py-1 border rounded-lg text-sm hover:bg-gray-50 transition"
                            >
                                {viewMode === 'week' ? (
                                    <>
                    ...
                    {viewMode === 'week' ? (
                        ...
                        <p className="text-gray-600 text-sm">
                            ...
                            {selectedDay.toDateString() === new Date().toDateString() && (
                                <span className="ml-2 text-blue-600 font-semibold">(Aujourd'hui)</span>
                            )}
                        </p>
                    )}
                </div>
                    ...
                    <>
                        {viewMode === 'multitrack' ? (
                            <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
                                <div className="grid border-b" style={{ gridTemplateColumns: `100px repeat(${rooms.length}, 1fr)` }}>
                                    <div className="p-3 bg-gray-50 font-semibold text-sm text-gray-600 border-r">
                                        Horaire
                                    </div>
                                    {rooms.map(room => (
                                        <div key={room.id} className="p-3 bg-gray-50 font-semibold text-sm text-gray-700 text-center border-r last:border-r-0">
                                            {room.name}
                                        </div>
                                    ))}
                                </div>
                                            ...
                                            <div className="border-r">
                                                {Array.from({ length: 14 }, (_, i) => i + 7).map(hour => (
                                                    <div key={hour} className="border-b border-gray-100 h-16 relative">
                                                        <span className="absolute -top-3 left-2 text-xs text-gray-400 bg-white px-1">
                                                            {hour}:00
                                                        </span>
                                                    </div>
                                                ))}
                                            ...
                                            {rooms.map(room => {
                                                ...
                                                return (
                                                    <div key={room.id} className="relative border-r last:border-r-0">
                                                        {Array.from({ length: 14 }, (_, i) => i + 7).map(hour => (
                                                            <div key={hour} className="border-b border-gray-100 h-16"></div>
                                                        ))}
